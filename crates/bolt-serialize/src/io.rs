...
use crate::manifest::{SHARDS_DIR, TensorEntry, TensorLocation, dtype_to_name};
use crate::shard::{
    LoadedShard, ShardPlan, compute_bytes_checksum, compute_file_checksum, plan_shards,
    verify_checksum, write_shard,
};
use crate::tensor_set::TensorIndex;
...
pub(crate) fn build_tensor_entries<'a>(
    tensor_entries: &mut BTreeMap<String, TensorEntry>,
    tensors: &[TensorToSave<'a>],
    plan: &ShardPlan,
    checksum_enabled: bool,
) -> Result<()> {
    let tensor_by_name: HashMap<&str, &TensorToSave<'a>> = tensors
        .iter()
        .map(|tensor| (tensor.meta.name.as_str(), tensor))
        .collect();

    for (shard_idx, shard) in plan.shards.iter().enumerate() {
        for name in &shard.tensor_names {
            let tensor = tensor_by_name[name.as_str()];
            let tensor_checksum = if checksum_enabled {
...
pub(crate) fn read_and_parse_manifest<T: for<'de> Deserialize<'de>>(path: &Path) -> Result<T> {
...
pub(crate) fn build_tensor_index(
    tensor_entries: &BTreeMap<String, TensorEntry>,
    shard_files: &[String],
    dir: &Path,
    corrupted_shards: &[usize],
) -> Result<HashMap<String, TensorIndex>> {
