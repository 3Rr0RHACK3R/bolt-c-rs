...
#[test]
fn step_dedupes_duplicate_params_by_name() {
    ...
    // The optimizer must deduplicate params by name, so only a single update is applied:
...
#[test]
fn velocity_state_keyed_by_param_name() {
    let backend = Arc::new(CpuBackend::new());
    let store = Store::<B, D>::new(backend.clone(), 0);
    ...
    // Velocity keyed by parameter name
    assert!(opt.velocity_state().contains_key(p.key()));
}
