...
fn test_broadcast_to_scalar() {
    let shape = Shape::from_slice(&[]).unwrap();
    let layout = Layout::contiguous(shape);
    let new_shape = Shape::from_slice(&[2, 3]).unwrap();
    let new_layout = layout.broadcast_to(&new_shape).unwrap();
    assert_eq!(new_layout.shape().as_slice(), &[2, 3]);
...
fn test_broadcast_to_matrix() {
    let shape = Shape::from_slice(&[2, 1]).unwrap();
    let layout = Layout::contiguous(shape);
    let new_shape = Shape::from_slice(&[2, 3]).unwrap();
    let new_layout = layout.broadcast_to(&new_shape).unwrap();
    assert_eq!(new_layout.shape().as_slice(), &[2, 3]);
...
fn test_iter_offsets_negative_stride() {
    let shape = Shape::from_slice(&[5]).unwrap();
...
fn test_broadcast_to_invalid() {
    let shape = Shape::from_slice(&[2, 3]).unwrap();
    let layout = Layout::contiguous(shape);
    let new_shape = Shape::from_slice(&[3]).unwrap();
    assert!(layout.broadcast_to(&new_shape).is_err());
}

#[test]
fn test_transpose_negative_axes() {
    let shape = Shape::from_slice(&[2, 3, 4]).unwrap();
    let layout = Layout::contiguous(shape);

    let transposed = layout.transpose(-2, -1).unwrap();
    assert_eq!(transposed.shape().as_slice(), &[2, 4, 3]);

    let transposed2 = layout.transpose(0, -1).unwrap();
    assert_eq!(transposed2.shape().as_slice(), &[4, 3, 2]);
}
...
fn test_permute_negative_axes() {
    ...
    assert_eq!(permuted.shape().as_slice(), &[4, 3, 2]);

    let permuted2 = layout.permute(&[0, -1, 1]).unwrap();
    assert_eq!(permuted2.shape().as_slice(), &[2, 4, 3]);
}

#[test]
fn test_permute_mixed_negative_positive() {
    let shape = Shape::from_slice(&[2, 3, 4, 5]).unwrap();
    let layout = Layout::contiguous(shape);

    let permuted = layout.permute(&[0, -1, 1, 2]).unwrap();
    assert_eq!(permuted.shape().as_slice(), &[2, 5, 3, 4]);
}
...
fn test_unsqueeze_axis_contiguous() {
    ...
    assert_eq!(unsqueezed.shape().as_slice(), &[2, 1, 3]);
...
fn test_unsqueeze_axis_general_layout() {
    ...
    let unsqueezed = layout.unsqueeze_axis(-1).unwrap();
    assert_eq!(unsqueezed.shape().as_slice(), &[4, 3, 2, 1]);
    assert!(!unsqueezed.is_contiguous());
}
