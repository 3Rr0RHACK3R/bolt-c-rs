...
fn test_scalar_indexing_rank_drop() -> Result<()> {
    ...
    let row = tensor.i(1)?;
    assert_eq!(row.shape().as_slice(), &[2]);
    assert_eq!(row.to_vec()?, vec![3.0, 4.0]);

    // x.i(1).i(0) -> scalar 3.0
    let scalar = row.i(0)?;
    assert_eq!(scalar.shape().as_slice(), &[]); // scalar tensor (rank 0)
...
fn test_range_variants() -> Result<()> {
    ...
    // x.i(..) -> all
    let slice = tensor.i(..)?;
    assert_eq!(slice.shape().as_slice(), &[6]);
...
fn test_tuple_indexing() -> Result<()> {
    ...
    assert_eq!(slice.shape().as_slice(), &[2, 2]);
...
fn test_partial_tuple_padding() -> Result<()> {
    ...
    assert_eq!(slice_tup.shape().as_slice(), &[3, 2]);
    ...
    assert_eq!(slice.shape().as_slice(), &[1, 2]); // dim0 dropped, dim1(1), dim2(2)
