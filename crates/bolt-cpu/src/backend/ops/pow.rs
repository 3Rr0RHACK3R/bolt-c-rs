...
impl PowKernel for f32 {
    fn pow_kernel(
        ...
        let mut out_storage: CpuStorage<Self> = alloc.allocate(numel)?;
            ...
            && rhs_layout.offset_bytes() == 0
        {
            ...
            let lhs_iter = lhs_layout.iter_offsets(Self::DTYPE)?;
            let rhs_iter = rhs_layout.iter_offsets(Self::DTYPE)?;
